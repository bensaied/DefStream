...
import { logout, detectIP } from "../../actions/user";
  ...
  useEffect(() => {
    const fetchIpAddress = async () => {
      try {
        ...
        const ip = data.ip;
        //console.log("This is the Server IP: ", ip);
        detectIP({ ip });
      } catch (error) {
        console.error(error);
      }
    ...
    <div className={classes.root}>
      ...
      >
        ...
        >
         ...
         {auth.user && auth.user.userType === "admin" ? (
          missions.mission
          .filter((mission) => mission.status === "Live") &&  missions.mission.length === 0 ? (
              <Alert
                severity="error"
          ...
          ) : auth.user && auth.user.allowedMissions.length === 0 &&
              auth.user.allowedMissions
                .filter((allowedMissions) => allowedMissions.status === "Live") ?
                (
                  <Alert
                    severity="error"
                    className={classes.alignItemsAndJustifyContent}
        ...
        >
          {auth.user && auth.user.userType === "admin" ? (
            missions.mission.filter((mission) => mission.status === "Live")
              .length === 0 ? (
              <Alert
                severity="error"
                className={classes.alignItemsAndJustifyContent}
              ...
              missions.mission
                .filter((mission) => mission.status === "Live")
                .map((mission) => (
                  <Card
                    key={mission.name + auth.user.name.toString()}
                    username={auth.user.name}
